...
contract Token is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    address public whitelistedLockContract;
    ...
    function setWhitelist(address _to) public onlyOwner {
        require(whitelistedLockContract == address(0), 'Already whitelisted');
        whitelistedLockContract = _to;
    }
    ...
    function transferFrom(
        ...
        if (isContract(_to)) {
            require(
                whitelistedLockContract == _to,
                'Smart contract not whitelisted'
            );
            _approve(
                _from,
                msg.sender,
